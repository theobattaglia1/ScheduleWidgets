...
final class BackgroundScheduler {
    ...
    #else
    // macOS: Background tasks not available, provide no-op implementations
    func registerBackgroundTasks() {
        log("Background tasks not available on macOS")
    }
    ...
    func performFullRefresh() async throws {
        ...
        // Fetch events for 14 days (to cover Next Week view which can be up to ~13 days out)
        print("[BackgroundScheduler] Fetching events...")
        let events = try await calendarService.fetchUpcomingEvents(days: 14)
